...
{    
        ...
        for (auto lb_idir: block_range*range(0, ar_grid.dim()))
        {
            ...
            for (auto idx: grid_range)
            {
                ...
                utils::foreach_param([&](const auto& flux_func)
                {
                    ...
                    auto flux_data_alias = omni::interpret_stencil<omni_type>(flux_data);
                    auto flux = flux_func(flux_data_alias);
    ...
    */
        ...
        const flux_funcs_t&... flux_funcs)
    {
        ...
        flux_div_OLD(prims, rhs, policy, domain_boundary_flux, flux_funcs...);
    }
    */
