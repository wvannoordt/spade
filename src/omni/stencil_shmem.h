...
namespace spade::omni
{
    ...
    template <typename omni_type, typename array_t>
    requires (omni_type::center() == grid::face_centered)
    inline auto get_shmem(int nx, int ny, int nz, const array_t&)
    {
        // THIS WILL NEED TO BE FIXED!!!!!
        // This will cause someone a lot of grief at some point. I actually need to combine all cell infos into a single list.
        using full_face_info_list = typename omni_type::info_at<omni::offset_t<0, 0, 0>>;
        ...
        int border_size = utils::max(left_extent, right_extent);
        
        std::size_t face_size = nx*ny*nz;
        std::size_t cell_size = (nx + 2*border_size)*ny*nz;
        return dispatch::shmem::make_shmem(dispatch::shmem::vec<face_data_type>(face_size), dispatch::shmem::vec<cell_data_type>(cell_size));
    }
}