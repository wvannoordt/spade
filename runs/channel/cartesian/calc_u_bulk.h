...
real_t calc_u_bulk(const auto& q, const auto& air)
{
    ...
    struct integrate_rho_u_t
    {
        ...
        integrate_rho_u_t(const spade::fluid_state::perfect_gas_t<real_t>& gas_in, const real_t& dV_in) {gas = &gas_in; dV = dV_in;}
    ...
    struct integrate_rho_t
    {
        ...
        const spade::fluid_state::perfect_gas_t<real_t>* gas;
        typedef prim_t arg_type;
        integrate_rho_t(const spade::fluid_state::perfect_gas_t<real_t>& gas_in, const real_t& dV_in) {gas = &gas_in; dV = dV_in;}
        real_t operator () (const prim_t& q) const
        {
            real_t rho = q.p()/(gas->get_R(q)*q.T());
            return rho*dV;
