...
int main(int argc, char** argv)
{
    ...
    auto mms_conv_func = [&](const v3d& xyz) -> spade::ctrs::array<real_t,5>
    {
    ...
    // spade::convective::weno_3 tscheme(air);
    ...
    for (int n = 0; n < numcells.size(); ++n)
    {
        ...
        err_0_l2[n]   = sqrt(spade::algs::transform_reduce(rhs, [&](const spade::ctrs::array<real_t, 5>& rhs_arr) -> real_t {return rhs_arr[0]*rhs_arr[0];}, sum)/num_dof);
        ...
        err_0_linf[n] = spade::algs::transform_reduce(rhs, [&](const spade::ctrs::array<real_t, 5>& rhs_arr) -> real_t {return std::abs(rhs_arr[0]);}, max);
