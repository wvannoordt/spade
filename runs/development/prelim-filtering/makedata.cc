...
int main(int argc, char** argv)
{
    ...
    spade::grid::grid_array rhs0 (grid, 0.0, spade::dims::static_dims<5>(), spade::dims::singleton_dim());
    spade::grid::grid_array rhs1 (grid, 0.0, spade::dims::static_dims<5>(), spade::dims::singleton_dim());
    ...
    if (init_from_file)
    {
        spade::output::binary_read(init_filename, prim);
    }
    
    spade::output::output_vtk("output", "ini", grid, prim);
    ...
    auto calc_rhs = [&](auto& rhs, auto& q, const auto& t) -> void
    {
        ...
        spade::flux_algs::flux_lr_diff(q, rhs, tscheme);
        // spade::flux_algs::flux_lr_diff(q, rhs, wscheme);
        // spade::flux_algs::flux_lr_diff(prim, rhs, visc_scheme);
    ...
    for (auto nti: range(0, nt_max))
    {
        ...
        if (nt%nt_skip == 0)
        {
            ...
            spade::output::output_vtk("output", filename, grid, prim);
        ...
        if (nt%checkpoint_skip == 0)
        {
            ...
            spade::output::binary_write(filename, prim);
            if (group.isroot()) print("Done.");
        }
        time_int.advance();
        if (std::isnan(umax))
